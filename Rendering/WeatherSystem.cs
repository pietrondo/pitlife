...
public sealed class WeatherSystem
{
    ...
    {
        var isSnow = climate.CurrentSeason == Season.Winter || climate.TemperatureModifier < -0.05f;
        var isRain = !isSnow && (climate.CurrentSeason == Season.Autumn || climate.CurrentSeason == Season.Spring);
        ...
        var windDir = climate.WindDirection;
        var windSpeed = climate.WindSpeed;
        ...
        for (var i = _count - 1; i >= 0; i--)
        {
            ...
            var sway = isSnow ? MathF.Sin(p.Life * 3f + p.Position.X * 0.01f) * 20f : 0f;
            var fallSpeed = isSnow ? 40f + _rng.NextSingle() * 60f : 250f + _rng.NextSingle() * 200f;
        ...
        var viewLeft = camera.Position.X - camera.ViewportWidth / (2f * camera.Zoom);
        var viewRight = camera.Position.X + camera.ViewportWidth / (2f * camera.Zoom);
        var viewTop = camera.Position.Y - camera.ViewportHeight / (2f * camera.Zoom);
        var viewHeight = camera.ViewportHeight / camera.Zoom;

        var spawnPerFrame = isSnow ? 4 : (isRain ? 8 : 0);
        for (var i = 0; i < spawnPerFrame && _count < target; i++)
        {
            var x = viewLeft + _rng.NextSingle() * (viewRight - viewLeft);
            var y = viewTop - _rng.NextSingle() * viewHeight * 0.5f;
            var life = isSnow ? 3f + _rng.NextSingle() * 5f : 0.5f + _rng.NextSingle() * 0.8f;
    ...
    public void Draw(SpriteBatch sb, Texture2D pixel, bool isSnow)
    {
        for (var i = 0; i < _count; i++)
        {
            ...
            var px = (int)p.Position.X;
            var py = (int)p.Position.Y;
            if (isSnow)
            {
            ...
            {
                var len = (int)(4 + p.Velocity.Y * 0.01f);
