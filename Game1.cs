...
public class Game1 : Game
{
    ...
    internal static void SaveLanguagePref()
    {
        try
        {
            File.WriteAllText("settings.json", System.Text.Json.JsonSerializer.Serialize(new { language = I18n.CurrentLanguage }));
        }
        catch (Exception ex) { Logger.Error($"Failed to save language preference: {ex.Message}"); }
    }
