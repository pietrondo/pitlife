...
internal sealed class FeaturePlacer
{
    ...
    public void PlaceCoralReefs()
    {
        ...
        {
            var latFactor = Math.Abs(y / (float)_world.Height - 0.5f) * 2f;
    ...
    {
        for (var y = 1; y < _world.Height - 1; y++)
        {
            for (var x = 1; x < _world.Width - 1; x++)
            {
    ...
    {
        var placed = 0;
        var maxVolcanoes = Math.Max(1, _world.Width * _world.Height / 3000);
        for (var attempt = 0; attempt < 100 && placed < maxVolcanoes; attempt++)
        {
            var x = _rng.Next(1, _world.Width - 1);
            var y = _rng.Next(1, _world.Height - 1);
